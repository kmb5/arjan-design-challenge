...
def handle_payment(total: Decimal) -> None:
    payment_methods = plugin_manager.all_plugins()

    payment_type = input(
        f"What payment method would you like to use? ({'/'.join(payment_methods)}) "
    )

    try:
        payment_handler = plugin_manager.get_plugin(payment_type)
...
def main() -> None:
    plugin_manager.load_plugins_from_folder(PLUGINS_FOLDER)
