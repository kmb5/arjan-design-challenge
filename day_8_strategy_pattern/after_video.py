...
def process_payment_cc(amount: Decimal) -> None:
...
def process_payment_paypal(amount: Decimal) -> None:
    email = input("Please enter your PayPal email address: ")
    password = input("Please enter your PayPal password: ")
    password_masked = len(password) * "*"
    print(
        f"Processing PayPal payment of ${amount:.2f} with email {email} and password {password_masked}..."
    )


def process_payment_apple(amount: Decimal) -> None:
...
PAYMENT_HANLDERS: dict[str, PaymentHandlerFn] = {
...
def handle_payment(total: Decimal) -> None:
    payment_handlers = PAYMENT_HANLDERS.keys()

    payment_type = input(
        f"What payment method would you like to use? ({'/'.join(payment_handlers)}) "
    )
    try:
        PAYMENT_HANLDERS[payment_type](total)
    except KeyError:
        print(f"Invalid payment type '{payment_type}'!")
